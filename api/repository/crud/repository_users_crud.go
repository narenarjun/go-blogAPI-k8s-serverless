...
func (r *repositoryUsersCRUD) FindAll() ([]models.User, error) {
	...
	go func(ch chan<- bool) {
		defer close(ch)
		err = r.db.Model(&models.User{}).Limit(100).Find(&users).Error
		if err != nil {
			ch <- false
			return
		}
...
func (r *repositoryUsersCRUD) FindByID(uid uint32) (models.User, error) {
	...
	go func(ch chan<- bool) {
		defer close(ch)
		err = r.db.Model(&models.User{}).Where("id = ?", uid).Take(&user).Error
		if err != nil {
			ch <- false
			return
		}
...
func (r *repositoryUsersCRUD) Update(uid uint32, user models.User) (int64, error) {
	...
	go func(ch chan<- bool) {
		defer close(ch)
		rs = r.db.Model(&models.User{}).Where("id = ?", uid).Take(&models.User{}).UpdateColumns(
...
func (r *repositoryUsersCRUD) Delete(uid uint32) (int64, error) {
	...
	go func(ch chan<- bool) {
		defer close(ch)
		rs = r.db.Model(&models.User{}).Where("id = ?", uid).Take(&models.User{}).Delete(&models.User{})
		ch <- true
	}(done)
